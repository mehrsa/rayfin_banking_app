...
function isExpense(transaction: BankTransaction): boolean {
  return (
...
export function getTransactionImpact(
  ...
  const outgoing =
    transaction.fromAccount_id === accountId ? transaction.amount : 0;
  ...
  return [...accounts]
    ...
    .map((account) => {
      const netChange = transactions.reduce(
        (total, transaction) => total + getTransactionImpact(transaction, account.id),
        0
      );
...
function buildCategorySpend(
  ...
  const expenseTransactions = transactions.filter(
    (transaction) => isExpense(transaction) && isCurrentMonth(toDate(transaction.postedAt))
  );
  ...
  return categories
    ...
    .map((category) => {
      ...
      const utilization =
        category.monthlyBudget > 0 ? spent / category.monthlyBudget : 0;
      const exceededAmount =
        category.monthlyBudget > 0 ? Math.max(spent - category.monthlyBudget, 0) : 0;
...
function buildTopMerchants(transactions: BankTransaction[]): MerchantSpend[] {
  ...
  return [...merchantMap.values()]
    .map((merchant) => ({
      ...merchant,
      shareOfExpenses:
        totalExpenses > 0 ? (merchant.amount / totalExpenses) * 100 : 0,
    }))
    .sort((left, right) => right.amount - left.amount)
    .slice(0, 5);
}

function buildBudgetHealthSummary(categorySpend: CategorySpend[]): BudgetHealthSummary {
...
export function buildBankingAnalytics(
  ...
  const averageMonthlyIncome =
    monthlyCashFlow.reduce((total, point) => total + point.income, 0) /
    Math.max(monthlyCashFlow.length, 1);
  const averageMonthlyExpenses =
    monthlyCashFlow.reduce((total, point) => total + point.expenses, 0) /
    Math.max(monthlyCashFlow.length, 1);
  const averageMonthlyNet =
    monthlyCashFlow.reduce((total, point) => total + point.net, 0) /
    Math.max(monthlyCashFlow.length, 1);
  ...
  return {
    ...
    recentTransactions: [...snapshot.transactions].sort(
      (left, right) =>
        toDate(right.postedAt).getTime() - toDate(left.postedAt).getTime()
    ),
