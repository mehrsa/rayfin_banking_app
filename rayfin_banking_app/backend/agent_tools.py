...
def heuristic_route(prompt: str, editing_widget: dict[str, Any] | None) -> str:
    ...
    if any(
        token in lowered
        for token in ('create account', 'open account', 'transfer', 'move money', 'deposit', 'withdraw')
    ):
...
def refresh_widget(widget: dict[str, Any], analytics: dict[str, Any]) -> dict[str, Any]:
    ...
    refreshed['last_refreshed'] = __import__('datetime').datetime.utcnow().isoformat()
    return refreshed
...
def build_widget_from_intent(
    ...
    widget_id = (existing_widget or {}).get('id', f'widget-{uuid.uuid4().hex[:10]}')
    ...
    widget = {
        ...
        'last_refreshed': __import__('datetime').datetime.utcnow().isoformat(),
        'config': {
